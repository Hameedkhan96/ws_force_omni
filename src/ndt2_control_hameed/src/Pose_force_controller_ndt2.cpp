...
/*
		...
		/*-----FST Control----------*/
		fx =  (2*tanh(ex) + 4*dex + 0.05*tanh(vx));
		//fy = -(1.5*tanh(ey) + 2*dey + 0.1*tanh(vy));
		fy = -(8.0*tanh(ey) + 12*dey + 0.1*tanh(vy));
		fz =  140*tanh(ez) + 23*dez + 12*tanh(vz);  
...
/* 
	   ...
	   fx =  (5*tanh(ex) + 4*dex + 0.1*tanh(vx));
		fy = -(1.5*tanh(ey) + 2*dey + 0.1*tanh(vy));
		fz =  80*tanh(ez) + 40*dez + 10*tanh(vz);      
		//fz =  100*tanh(ez) + 40*dez + 10*tanh(vz);                       
	    //fz =  400*tanh(ez)+200*dez+100*tanh(vz);                      
		tx =   2*dephi + 10*ephi;
		ty = -(2*dethe + 10*ethe + 0.3*vthe);
		tz =   2*depsi + 10*epsi;
		//u<<0*fx,0*fy,fz,0*tx,0*ty,0*tz;
		u<<fx,fy,fz,tx,ty,tz;
*/
		...
		/*-----Publishing----------*/
		if(take_off == 1.0){
			t_msg1.data =  db(a[0],1e-3);                     // should we need another 4 ?
			t_msg2.data =  db(a[1],1e-3);
			t_msg3.data =  db(a[2],1e-3);
			t_msg4.data =  db(a[3],1e-3);
			
			motor_vel.data[1] = sat( sqrt(T[0]/kf),1100 );        // front-left up--2--CW,
			motor_vel.data[2] = sat( sqrt(T[1]/kf),1100 );        // backt-left up--3--CCW
			motor_vel.data[3] = sat( sqrt(T[2]/kf),1100 );        // back-left down--8-- CW
			motor_vel.data[0] = sat( sqrt(T[3]/kf),1100 );        // front-right up--1-- CCW
			motor_vel.data[4] = sat( sqrt(T[0]/kf),1100 );        // backt-left up--3--CCW
			motor_vel.data[7] = sat( sqrt(T[1]/kf),1100 );        // back-right up--4 --CW
			motor_vel.data[6] = sat( sqrt(T[2]/kf),1100 );        // back-right down--7-- CCW
			motor_vel.data[5] = sat( sqrt(T[3]/kf),1100 );        // front-right down--6-- CW


			//motor_vel.data[6] = sat( sqrt(T[0]/kf),1100 );        // back-right down--7-- CCW
			//motor_vel.data[5] = sat( sqrt(T[1]/kf),1100 );        // front-right down--6-- CW
			//motor_vel.data[7] = sat( sqrt(T[2]/kf),1100 );        // back-left down--8-- CW
			//motor_vel.data[4] = sat( sqrt(T[3]/kf),1100 );        // front-left down--5-- CCW
			//motor_vel.data[2] = sat( sqrt(T[0]/kf),1100 );        // backt-left up--3--CCW
			//motor_vel.data[1] = sat( sqrt(T[1]/kf),1100 );        // front-left up--2--CW
			//motor_vel.data[3] = sat( sqrt(T[2]/kf),1100 );        // back-right up--4 --CW
			//motor_vel.data[0] = sat( sqrt(T[3]/kf),1100 );        // front-right up--1-- CCW
			...
			t_msg3.data =  0.;
			t_msg4.data =  0.;	
			motor_vel.data[2] = 0.;             // the same here?
			motor_vel.data[1] = 0.;
			motor_vel.data[3] = 0.;
			motor_vel.data[0] = 0.;
		...
		cout<<"rotor"<<1<<" = "<<motor_vel.data[1]<<endl;
		cout<<"rotor"<<2<<" = "<<motor_vel.data[2]<<endl;
...
void save(){
	ofstream myfile;
	myfile.open ("resultsTanh.txt",std::ios::app);
	myfile <<x<<","<<y<<","<<z<<","<<phi<<","<<the<<","<<psi<<","
	<<xd<<","<<yd<<","<<zd<<","<<phid<<","<<thed<<","<<psid<<endl;
	myfile.close();
}*/
...
void save() {
    ...
    if (firstWrite) {
        myfile.open("resultsTanh.txt", std::ios::trunc);  // Open in trunc mode to clear the file
        firstWrite = false;  // Set to false after first write
    } else {
        myfile.open("resultsTanh.txt", std::ios::app);  // Open in append mode for subsequent writes
    }

    myfile << xd << "," << yd << "," << zd << "," << x << "," << y << "," << z << endl;
    myfile.close();
}
