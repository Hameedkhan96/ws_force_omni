...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		take_off = 1.0; // references.data[18];
		...
		if(take_off == 1.0){
			...
			motor_vel.data[4] = 0*sat( sqrt(T[0]/kf),1100 );        // backt-left up--3--CCW
			motor_vel.data[7] = 0*sat( sqrt(T[1]/kf),1100 );        // back-right up--4 --CW
			motor_vel.data[6] = 0*sat( sqrt(T[2]/kf),1100 );        // back-right down--7-- CCW
			motor_vel.data[5] = 0*sat( sqrt(T[3]/kf),1100 );        // front-right down--6-- CW
